# 🧠 PMP MASTER MINDMAP SYSTEM

> A structured knowledge system to master PMP Exam Prep  
> Built for **thinking – linking – practicing – passing**

---

# 📦 SYSTEM STRUCTURE

```diagram
PMP/
│
├── 0_MASTER_MINDMAP/
│   └── PMP_MASTER_MINDMAP.md
│
├── 1_COURSES/
│   ├── Course_1_Principles.md
│   ├── Course_2_People.md
│   ├── Course_3_Process.md
│   └── Course_4_Business.md
│
├── 2_CONCEPTS_LIBRARY/
│   ├── 01_Fundamentals/
│   ├── 02_People/
│   ├── 03_Process/
│   ├── 04_Agile_Hybrid/
│   └── 05_Business/
│
├── 3_PMI_MINDSET/
│   ├── PMI_CORE_RULES.md
│   └── DECISION_FRAMEWORK.md
│
├── 4_EXAM_PRACTICE/
│   ├── Situations.md
│   ├── Mistakes_Log.md
│   ├── Patterns.md
│   └── Mock_Analysis.md
│
└── 5_REVISION/
    ├── Cheat_Sheet.md
    ├── Flashcards.md
    └── Last_Week_Review.md
```


---

# 🧭 MASTER NAVIGATION

👉 Central Brain  
📌 [[0_MASTER_MINDMAP/PMP_MASTER_MINDMAP.md|Open PMP Control Center]]

---

# 🎓 COURSE BREAKDOWN

## 👥 PEOPLE
📚 [[1_COURSES/Course_2_People.md|Course 2 – People Management]]

- Leadership  
- Team Development  
- Stakeholder Engagement  
- Communication  

---

## ⚙️ PROCESS
📚 [[1_COURSES/Course_3_Process.md|Course 3 – Process Management]]

- Initiation → Charter  
- Planning → Scope, Schedule, Cost, Risk  
- Execution → Team, Quality  
- Monitoring → Change, Performance  
- Closing  

---

## 🏢 BUSINESS ENVIRONMENT
📚 [[1_COURSES/Course_4_Business.md|Course 4 – Business Environment]]

- Compliance  
- Value Delivery  
- Benefits Realization  
- Strategy Alignment  

---

## 🔄 APPROACHES
📚 [[1_COURSES/Course_1_Principles.md|Course 1 – Principles & Approaches]]

- Predictive  
- Agile  
- Hybrid  

---

# 🧠 KNOWLEDGE SYSTEM

## 📚 Concepts Library
📂 [[2_CONCEPTS_LIBRARY/|Open Concepts Library]]

- Fundamentals  
- People  
- Process  
- Agile / Hybrid  
- Business  

---

## 🧠 PMI Mindset
📂 [[3_PMI_MINDSET/|Open PMI Mindset]]

- Core Rules → [[3_PMI_MINDSET/PMI_CORE_RULES.md|PMI Core Rules]]  
- Decision Engine → [[3_PMI_MINDSET/DECISION_FRAMEWORK.md|Decision Framework]]  

---

## 🎯 Practice Zone
📂 [[4_EXAM_PRACTICE/|Open Practice Zone]]

- Situations  
- Patterns  
- Mistakes Log  
- Mock Analysis  

---

## 🔁 Revision Zone
📂 [[5_REVISION/|Open Revision Zone]]

- Cheat Sheet  
- Flashcards  
- Final Review  

---

# ⚡ STUDY FLOW (MOST IMPORTANT)

1. 📺 Learn → [[1_COURSES/|Go to Courses]]  
2. 🧠 Extract → [[2_CONCEPTS_LIBRARY/|Build Knowledge]]  
3. 🔗 Link → [[0_MASTER_MINDMAP/|Connect Mindmap]]  
4. 🎯 Practice → [[4_EXAM_PRACTICE/|Practice Questions]]  
5. 🔁 Review → [[5_REVISION/|Final Revision]]  

---

# 🔥 QUICK RECALL

- People → Communicate + Support  
- Process → Follow flow + Control change  
- Business → Deliver value  
- Agile → Facilitate, not command  

---

# 🚨 EXAM THINKING PATTERN

1. Identify domain (People / Process / Business)  
2. Check approach (Agile / Predictive)  
3. Analyze situation  
4. Apply PMI mindset  
5. THEN act  

---

# 🎯 FINAL NOTE

👉 This system = **Second Brain for PMP**  
👉 Focus on **linking, not duplicating**  
👉 Master the **thinking pattern**, not just knowledge  